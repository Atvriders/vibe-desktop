...
**Model:** Claude **Haiku 4.5** with thinking disabled — the fastest tier, because this is a latency-sensitive UI loop. Expect **~1.5–2 seconds per click**: it feels like a charming, slightly-laggy hallucinated OS, not a native one. That latency is inherent to "no code behind it" — every interaction is a model round trip.
...
- **It's slow-ish.** ~1.5–2s per click — a model round trip every time. Great for a demo, not a daily driver.
